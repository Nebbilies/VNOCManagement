...
export function UnclaimMatchButton({ matchId }: Props) {
    ...
    const handleUnclaim = async () => {
        ...
        setLoading(true);
        const response = await fetch(`${apiBase}/matches/unclaim`, {
            method: 'POST',
            credentials: "include",
            headers: {
        ...
        if (!response.ok) {
            ...
            if (errorData.error) {
                showError("Failed to unclaim match: " + errorData.error);
            } else {
                showError('Failed to unclaim match');
            }
        }
        else {
            showSuccess('Match unclaimed successfully');
            setRefresh(true);
        }
        setLoading(false);
    };
    ...
    return (
        <>
            ...
            <div onMouseEnter={() => setHoveringClaimMatch(true)}
                ...
                <CircleMinus onClick={() => setShowModal(true)}
                    ...
                    <>
                        ...
                        >
                            <div className="bg-[#23263a] rounded-lg p-6 text-white">
                                <div>Are you sure you want to unclaim match <b>{matchId}</b> {user ? "as " + userRole : ""}?</div>
